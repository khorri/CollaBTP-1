...
module.exports.sockets = {
  ...
  ***************************************************************************/
  onConnect: function(session, socket) {
	 if (session.user) {
		 	User.subscribe(socket, session.user, 'message');
	   		User.update(session.user.id,{
				online: true
			}, function(err){
				if(err) return next(err);
				sails.io.sockets.emit('userLogin', {userId:session.user.id});
			});
	   }
  },
	  ...
	  if (session.user) {
	   if (User.subscribers(session.user.id).length < 1) {
	   		...
	   		User.update(session.user.id,{
				...
				if(err) return next(err);
				sails.io.sockets.emit('userLogout', {userId:session.user.id});
			});
		   //session.destroy();
	   }
	  }
